                                                          ...
                                                          BinaryNode<ItemType> *newNodePtr)
{
   ...
   if (left)
   {
      BinaryNode<ItemType> *newLeftPtr = newNodePtr;
      if (leftPtr != NULL)
      {
         newLeftPtr = randomAdd(leftPtr, newNodePtr);
      }
      subTreePtr->setLeftChildPtr(newLeftPtr);
      return subTreePtr;
   }

   BinaryNode<ItemType> *newRightPtr = newNodePtr;
   if (rightPtr != NULL)
   {
      newRightPtr = randomAdd(rightPtr, newNodePtr);
   }
   subTreePtr->setRightChildPtr(newRightPtr);
   return subTreePtr;
} // end randomAdd
...
template class BinaryNode<PersonItem>;

template class BinaryNodeTree<std::string>;
template class BinaryNodeTree<int>;
template class BinaryNodeTree<PersonItem>;