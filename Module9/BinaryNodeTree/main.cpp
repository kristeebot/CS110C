...
// Once you are up and running, examine the add() function in BinaryNodeTree.cpp.  It uses a balancedAdd() helper function to add a node, 
// ensuring that the resulting tree remains balanced.
// Instead of calling balancedAdd() here, create a new function randomAdd(), which adds nodes in random places in the tree, and use that function instead.  
// A random place should be determined by starting at the root, and traversing down the tree to the left or right with 50% probability each time.  
// When you get to a node in the tree where you would like to traverse down another level but there is no child in the direction you picked, insert the node at this location.
// Don't forget: this is a general binary tree - not a binary search tree!  

//**************PART TWO****************
/*
For part 2 of the assignment, no code is required.  
Answer the following questions about binary search trees (not just general binary trees).  
To help with partial credit, make sure you show the binary search tree after inserting each value
*/
